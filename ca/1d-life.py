...
def get_new_value(old_gen, old_automata):
    ...
    beginning = SQ_NUM * old_gen
    end = beginning + (SQ_NUM)
    insert = SQ_NUM * (old_gen + 1)

    if insert > SQ_NUM * SQ_NUM - SQ_NUM:
        return old_automata

    new_auto = old_automata[:]
    last_row = new_auto[beginning:end]

    for (i, x) in enumerate(last_row):

        last_top = insert - SQ_NUM + i
        last_left = last_top - 1
        last_right = last_top + 1

        last_sum = old_automata[last_top]
        if last_left >= 0 or last_left > end:
            last_sum += old_automata[last_left]

        if last_right < insert:
            last_sum += old_automata[last_right]

        if last_sum == 0 or last_sum == 3:
            new_auto[insert + i] = 0
        else:
            new_auto[insert + i] = 1
